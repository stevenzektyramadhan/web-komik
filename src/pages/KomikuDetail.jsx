...
function getBackLabel(from) {
  ...
  if (from.startsWith('/komiku')) return '← Kembali ke Komiku';
...
export default function KomikuDetail() {
  ...
  const isFavorit = favorit.some((f) => f.id === slug && f.source === 'komiku');
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [m, c] = await Promise.all([getKomikuDetail(slug), getKomikuChapters(slug)]);
        if (!active) return;
        setManga(m);
        setChapters(c);
      } catch (e) {
  ...
  const toggleFavorit = () => {
    if (!manga) return;
    if (isFavorit) {
      setFavorit(favorit.filter((f) => !(f.id === slug && f.source === 'komiku')));
    } else {
      setFavorit([
        ...favorit,
        { id: manga.slug, title: manga.title, cover: manga.cover, source: 'komiku' },
      ]);
    }
  };
      ...
      <div className="mb-8 flex flex-col gap-6 md:flex-row">
        ...
        <div className="w-48 shrink-0 overflow-hidden rounded-xl border border-gray-200 dark:border-dark-700 md:w-56">
          {manga.cover ? (
            ...
            <div className="flex aspect-[3/4] items-center justify-center bg-gray-100 text-5xl dark:bg-dark-800">
              📕
            </div>
          )}
        </div>
        ...
        <div className="flex-1">
          ...
          <div className="mt-3 flex flex-wrap gap-2 text-xs">
            {manga.type && (
              <span className="rounded bg-gray-100 px-2 py-1 dark:bg-dark-700">{manga.type}</span>
            )}
            {manga.rating && (
              <span className="rounded bg-gray-100 px-2 py-1 dark:bg-dark-700">⭐ {manga.rating}</span>
            )}
            {manga.views && (
              <span className="rounded bg-gray-100 px-2 py-1 dark:bg-dark-700">{manga.views}</span>
            )}
          ...
          <button
            ...
            className={`mt-4 ${isFavorit ? 'btn-ghost' : 'btn-primary'}`}
          >
        ...
        {chapters.length === 0 ? (
          ...
          <div className="overflow-hidden rounded-xl border border-gray-200 dark:border-dark-700">
            {chapters.map((c, idx) => (
              <Link
                key={c.chapter}
                to={`/komiku/${slug}/baca/${c.chapter}?cs=${encodeURIComponent(c.slug)}`}
